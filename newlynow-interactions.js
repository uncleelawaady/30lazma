...
(function () {
  ...
  ready(function () {
    ...
    if ('IntersectionObserver' in window) {
      ...
      q('.reveal').forEach(function (el) { el.classList.add('show'); });
    }
    ...
    /* ---------- 3) Meter bars: animate width from 0 ---------- */
    ...
    /* ---------- 4) Hero spotlight + light parallax ---------- */
    ...
    if (hero && !coarse && !reduce) {
      ...
      hero.addEventListener('pointermove', function (e) {
        ...
        orbs.forEach(function (o, i) {
          var f = (i + 1) * 6;
          o.style.transform = 'translate(' + ((px - .5) * f) + 'px,' + ((py - .5) * f) + 'px)';
        });
      });
      hero.addEventListener('pointerleave', function () {
        hero.classList.remove('spot');
        orbs.forEach(function (o) { o.style.transform = ''; });
      });
    }

    /* ---------- 5) Card tilt (subtle, ≤3deg) ---------- */
    if (!coarse && !reduce) {
  ...
  /* ---------- Toast helper (usable from other scripts) ---------- */
