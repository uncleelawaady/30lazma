...
ملف `.claude/settings.json` كان فيه **Hostinger API token مكتوب صريح**، والريبو عام. التوكن ده **مكشوف ولازم يتعتبر محروق**، وموجود كمان في كل commit في تاريخ الريبو.

الملف اتنضّف دلوقتي وبقى بيقرا التوكن من متغير بيئة `HOSTINGER_API_TOKEN`، لكن **تنضيف الملف مش بيبطّل التوكن**. الخطوات بالترتيب:

1. **Revoke أولًا** — hPanel → **API** → التوكن الحالي → Revoke. دي الخطوة الوحيدة اللي بتوقف الضرر فعلًا.
2. **اعمل توكن جديد** وحطه في متغير بيئة على جهازك بس:
   - Windows: `setx HOSTINGER_API_TOKEN "…"` وبعدين اقفل الـterminal وافتحه تاني.
   - macOS/Linux: `export HOSTINGER_API_TOKEN="…"` في `~/.zshrc` أو `~/.bashrc`.
   - متكتبوش في أي ملف جوّه الريبو، ومتبعتوش في شات ولا تحطه في لقطة شاشة.
3. **نضّف التاريخ** بعد الـRevoke — التوكن القديم لسه في كل commit، وGitHub بيفضل يقدّمه من الـcommit SHA حتى بعد الـforce-push.

> التاريخ ينضّف **بعد** الـRevoke مش قبله. أي حد نسخ الريبو قبل كده معاه التوكن أصلًا، فالنسخ مش بيرجع — التبطيل هو اللي بيرجع.
